[README.md](https://github.com/user-attachments/files/28623922/README.md)
# oluadun.github.io
Senior PM portfolio — Health IT, Clinical AI &amp; Insurtech
# Hi, I'm Olu Adun 👋

**Senior Product Manager · Health IT & Clinical AI · Insurtech · Regulated Enterprise Platforms**  
*CSM · CSPO · LSSGB*

---

## What I do

I build AI-powered products at the intersection of healthcare, data, and human workflow - where regulatory complexity meets the need for clarity, speed, and measurable impact.

Over 9 years, I've shipped products that clinicians actually use, payers actually trust, and engineers actually want to build.

| Impact Area | Result |
|---|---|
| 🤖 IDP Prior Auth Automation | $1.3M+ annual savings · 28% processing time reduction |
| 🩺 Clinical AI Assistant (0→1) | 30% paperwork reduction · 17–20% adoption gain |
| 📈 Product Roadmap Execution | ~25% ARR growth across clinical platforms |
| ⚡ Enterprise Agile Transformation | 28% faster time-to-market · $1M+ savings identified |

---

## Domain Expertise

```
Healthcare IT          FHIR · Prior Auth · EHR Integration · HIPAA · Utilization Management
Clinical AI            IDP · Agentic AI · LLM Products · Fraud Detection · Workflow Automation  
Insurtech              Claims Automation · Policy Admin · Payer/Provider Strategy
Data & Analytics       SQL · Tableau · Power BI · Funnel Analysis · A/B Testing · KPI Design
APIs & Systems         REST APIs · Microservices · System Design · API Lifecycle Management
Delivery               Scrum · SAFe · Kanban · OKRs · CI/CD · Lean Six Sigma
```

---

## Portfolio

| Project | Type | Outcome |
|---|---|---|
| [IDP Prior Authorization Automation](cases/idp-prior-auth.md) | Clinical AI · Healthcare | $1.3M+ savings · 28% faster processing |
| [Clinical AI Assistant — 0→1](cases/clinical-ai-assistant.md) | Product Launch · AI | 30% paperwork reduction |
| [FHIR EHR Integration MVP](cases/fhir-ehr-integration.md) | Interoperability · Health IT | Regional care gaps closed · HIPAA-2 compliant |
| [Enterprise Agile Transformation](cases/agile-transformation.md) | Delivery · Insurance | 28% faster TTM · $1M+ identified |

---

## Artifacts

Real PM deliverables — templates, frameworks, and documentation from actual product work.

- 📋 [PRD: IDP Automation](artifacts/prd-idp-automation.md)
- 📝 [User Stories: Clinical AI Assistant](artifacts/user-stories-clinical-ai.md)
- 🗺️ [Product Roadmap Framework](artifacts/roadmap-template.md)
- 🔍 [SQL: Clinical Data Analysis Queries](artifacts/sql-analysis.md)
- 🔄 [BPMN: Prior Auth Workflow](artifacts/bpmn-prior-auth.md) *(coming soon)*
- 🎯 [OKR Framework Template](artifacts/okr-framework.md) *(coming soon)*

---

## Background

**Product Manager | Product Owner · Ochsner Health** *(May 2022 – Present)*  
End-to-end product ownership for AI-enabled clinical platforms. Led 0→1 AI product development, FHIR API strategy, and autonomous fraud detection deployment across a major regional health system.

**Business Analyst | Scrum Master · State Farm** *(Feb 2017 – Apr 2022)*  
Led Agile transformation for 52-person insurance organization. Delivered $1M+ in SQL-driven cost savings and reduced ticket resolution time by 35%.

**Education:** M.S. Industrial Engineering — Louisiana State University | B.S. Civil Engineering — University of Ibadan

---

## Let's connect

📧 oluadun10@gmail.com  
💼 [linkedin.com/in/oluadun](https://linkedin.com/in/oluadun)  
🌐 [Portfolio Site](https://oluadun.github.io)

*Open to PM, PO, Senior PM, Technical PM and Product Analyst/ roles in Health IT, clinical AI, insurtech, and regulated fintech — remote or hybrid.*
